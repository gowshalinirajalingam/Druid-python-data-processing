...
data1 = open('past_10_days_<1GB.json')     #open interval written json file again
...
dfa.rename(columns={'Total_usage':'Total_usage_<1'}, inplace=True)
...
data2 = open('past_10_days_1GBTO5GB.json')     #open interval written json file again
...
dfb.rename(columns={'Total_usage':'Total_usage_1_5'}, inplace=True)
...
data3 = open('past_10_days_>5GB.json')     #open interval written json file again
...
dfc.rename(columns={'Total_usage':'Total_usage_>5'}, inplace=True)


#Join into one data frame
dff = dfa.join(dfb.set_index('timestamp'), on='timestamp')
dffinal = dff.join(dfc.set_index('timestamp'), on='timestamp')
...
dffinal.to_sql(con=db_conn, name='Past_10days_total_usage', if_exists='replace',index=False,chunksize=10000)

db_conn.close()
print("data set to db")
